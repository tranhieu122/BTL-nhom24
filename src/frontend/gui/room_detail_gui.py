...
from gui.theme import (_get_c, btn, labeled_entry)
...
class RoomDetailDialog(tk.Toplevel):
    ...
    def _build(self) -> None:
        # ── Header ──────────────────────────────────────────────────────────
        hdr = tk.Frame(self, bg=_get_c("ACCENT"), padx=20, pady=14)
        hdr.pack(fill="x")
        ...
        frm = tk.Frame(self, bg=_get_c("SURFACE"), padx=26, pady=20)
        ...
        status_wrap = tk.Frame(frm, bg=_get_c("SURFACE"), highlightthickness=1,
        ...
        # ── Hint line ────────────────────────────────────────────────────────
        hint = tk.Frame(frm, bg=_get_c("INFO_BG"), highlightthickness=1,
                        highlightbackground=_get_c("BORDER"))
        ...
        # ── Buttons ──────────────────────────────────────────────────────────
        btn_row = tk.Frame(self, bg=_get_c("SURFACE"), padx=26)
    ...
    def _save(self) -> None:
        ...
        # Validation
        if not v["room_id"]:
            messagebox.showwarning("Thieu thong tin", "Ma phong khong duoc de trong.", parent=self)
            return
        if not v["name"]:
            messagebox.showwarning("Thieu thong tin", "Ten phong khong duoc de trong.", parent=self)
            return
        if not v["capacity"]:
            messagebox.showwarning("Thieu thong tin", "Suc chua khong duoc de trong.", parent=self)
            return
        try:
            cap = int(v["capacity"])
            if cap <= 0:
            ...
            messagebox.showerror("Du lieu khong hop le",
                                 "Suc chua phai la so nguyen duong.", parent=self)
            return
        self.result = v
        self.destroy()
