...
from gui.theme import (_get_c, C_SURFACE, btn,
...
class ProfileDialog(tk.Toplevel):
    ...
    def _build(self) -> None:
        def _get_role_style(role: str) -> tuple[str, str, str]:
            if role == "Admin":
                return (_get_c("ACCENT"), _get_c("INFO_BG"), _get_c("ACCENT"))
            elif role == "Giang vien":
                return (_get_c("SUCCESS"), _get_c("SUCCESS_BG"), _get_c("SUCCESS"))
            else: # Sinh vien
                return (_get_c("WARNING"), _get_c("WARNING_BG"), _get_c("WARNING"))

        fg, bg, av_fill = _get_role_style(self.user.role)

        # Header with avatar
        hdr = tk.Frame(self, bg=_get_c("ACCENT"), padx=24, pady=18)
        hdr.pack(fill="x")

        av = tk.Canvas(hdr, width=60, height=60, bg=_get_c("ACCENT"),
                       highlightthickness=0)
        av.pack(side="left", padx=(0, 14))
        av.create_oval(2, 2, 58, 58, fill=av_fill, outline=_get_c("INFO_BG"), width=2)
        ...
        info = tk.Frame(hdr, bg=_get_c("ACCENT"))
        info.pack(side="left", fill="x")
        tk.Label(info, text=self.user.full_name, bg=_get_c("ACCENT"), fg="white", # type: ignore
                 font=("Segoe UI", 14, "bold")).pack(anchor="w")
        tk.Label(info, text=f"@{self.user.username}", bg=_get_c("ACCENT"), fg="#c7d2fe", # type: ignore
                 font=("Segoe UI", 10)).pack(anchor="w")
        badge = status_badge(info, self.user.role, bg=_get_c("ACCENT")) # type: ignore
        badge.config(bg=bg, fg=fg)
        badge.pack(anchor="w", pady=(4, 0))

        # Profile info
        body = tk.Frame(self, bg=_get_c("SURFACE"), padx=26, pady=18)
        body.pack(fill="x")

        def info_row(icon, label, value): # type: ignore
            row = tk.Frame(body, bg=_get_c("SURFACE"))
            row.pack(fill="x", pady=5)
            tk.Label(row, text=icon, bg=_get_c("SURFACE"), # type: ignore
                     font=("Segoe UI", 13), width=2).pack(side="left")
            tk.Label(row, text=f"{label}:", bg=_get_c("SURFACE"), fg=_get_c("MUTED"),
                     font=("Segoe UI", 9), width=14, anchor="w").pack(side="left")
            tk.Label(row, text=value, bg=_get_c("SURFACE"), fg=_get_c("TEXT"), # type: ignore
        ...
        # Divider
        tk.Frame(self, bg=_get_c("BORDER"), height=1).pack(fill="x", padx=24)

        # Change password section
        pw_frame = tk.Frame(self, bg=_get_c("SURFACE"), padx=26, pady=18)
