# login_gui.py  –  split-panel login screen  (login + register + forgot-pw)
from __future__ import annotations
import random as _rnd
import tkinter as tk
from tkinter import messagebox, ttk
from typing import Any
from gui.theme import (F_INPUT, btn, _get_c)

# ── Login screen branding colours (Dynamic) ──────────────────────────────────
def _get_lg_c(key: str) -> str:
    # Special lookup for login branding to maintain "Indigo" identity in light
    # but adapt to deep Slate/Navy in dark mode.
    from gui.theme import CURRENT_THEME
    if CURRENT_THEME == "dark":
        MAPPING = {
            "_LG_BG":    "BG",
            "_LG_MID":   "SURFACE",
            "_LG_GLOW":  "ACCENT",
            "_LG_TEXT":  "TEXT",
            "_LG_MUTED": "MUTED",
        }
        return _get_c(MAPPING.get(key, key))
    # Light mode: use fixed Indigo branding for the "Left Panel" to keep its identity
    BRANDING = {
        "_LG_BG":    "#1e1b4b", # Indigo 950
        "_LG_MID":   "#312e81", # Indigo 900
        "_LG_GLOW":  "#4f46e5", # Indigo 600
        "_LG_TEXT":  "#e0e7ff", # Indigo 100
        "_LG_MUTED": "#818cf8", # Indigo 400
    }
    return BRANDING.get(key, _get_c(key))

# ── Helpers ───────────────────────────────────────────────────────────────────

def _labeled_entry(parent: tk.Misc, label: str, var: tk.StringVar,
                   show: str = "", width: int = 32) -> tk.Frame:
    """Return a labeled, focus-bordered entry block."""
    tk.Label(parent, text=label, bg=_get_c("SURFACE"), fg=_get_c("MUTED"),
             font=("Segoe UI", 9, "bold")).pack(anchor="w", pady=(10, 0))
    wrap = tk.Frame(parent, bg=_get_c("SURFACE"), highlightthickness=1,
                    highlightbackground=_get_c("BORDER"))
    wrap.pack(fill="x", pady=(4, 0))
    e = tk.Entry(wrap, textvariable=var, width=width, show=show,
                 font=F_INPUT, relief="flat", bg=_get_c("SURFACE"), fg=_get_c("TEXT"))
    e.pack(padx=12, pady=8)

    def on_in(_: object) -> None:
        wrap.config(highlightbackground=_get_c("ACCENT"), highlightthickness=2)
    def on_out(_: object) -> None:
        wrap.config(highlightbackground=_get_c("BORDER"),  highlightthickness=1)
    e.bind("<FocusIn>",  on_in)  # type: ignore[arg-type]
    e.bind("<FocusOut>", on_out)  # type: ignore[arg-type]
    return wrap


# ── Register dialog ───────────────────────────────────────────────────────────

class RegisterDialog(tk.Toplevel):
    def __init__(self, master: tk.Misc, auth_controller: Any) -> None:
        super().__init__(master)
        self.auth_ctrl = auth_controller
        self.title("Dang ky tai khoan")
        self.resizable(False, False)
        self.configure(bg=_get_c("SURFACE"))
        self.transient(master)  # type: ignore[arg-type]
        self.grab_set()
        self._vars = {k: tk.StringVar() for k in
                      ("full_name", "username", "email", "phone",
                       "password", "confirm", "role")}
        self._vars["role"].set("Sinh vien")
        self._build()
        self.after(100, self._center)

    def _center(self):
        self.update_idletasks()
        pw = self.master.winfo_rootx() + self.master.winfo_width()  // 2
        ph = self.master.winfo_rooty() + self.master.winfo_height() // 2
        w, h = self.winfo_width(), self.winfo_height()
        self.geometry(f"+{pw - w//2}+{ph - h//2}")

    def _build(self) -> None:
        # ── Register dialog header (Indigo) ───────────────────────────────────
        hdr = tk.Frame(self, bg="#4f46e5", padx=24, pady=14)
        hdr.pack(fill="x")
        tk.Label(hdr, text="📝  Dang ky tai khoan moi",
                 bg="#4f46e5", fg="white",
                 font=("Segoe UI", 13, "bold")).pack(anchor="w")
        tk.Label(hdr, text="Tao tai khoan de dat phong hoc",
                 bg="#4f46e5", fg="#c7d2fe",
                 font=("Segoe UI", 9)).pack(anchor="w")

        body = tk.Frame(self, bg=_get_c("SURFACE"), padx=28, pady=20)
        body.pack(fill="x")

        _labeled_entry(body, "HO VA TEN *",        self._vars["full_name"])
        _labeled_entry(body, "TEN DANG NHAP *",    self._vars["username"])
        _labeled_entry(body, "EMAIL *",             self._vars["email"])
        _labeled_entry(body, "SO DIEN THOAI *",    self._vars["phone"])
        _labeled_entry(body, "MAT KHAU * (min 6)", self._vars["password"],  show="*")
        _labeled_entry(body, "XAC NHAN MAT KHAU",  self._vars["confirm"],   show="*")

        tk.Label(body, text="VAI TRO", bg=_get_c("SURFACE"), fg=_get_c("MUTED"),
                 font=("Segoe UI", 9, "bold")).pack(anchor="w", pady=(10, 0))
        ttk.Combobox(body, textvariable=self._vars["role"],
                     values=["Sinh vien", "Giang vien"],
                     state="readonly", width=30,
                     font=("Segoe UI", 10)).pack(fill="x", pady=(4, 0))

        # Notice
        notice = tk.Frame(body, bg=_get_c("INFO_BG"),
                          highlightthickness=1, highlightbackground=_get_c("BORDER"))
        notice.pack(fill="x", pady=(16, 0))
        tk.Label(notice, text="ⓘ  Tai khoan 'Admin' chi duoc tao boi quan tri vien.",
                 bg=_get_c("INFO_BG"), fg=_get_c("ACCENT"),
                 font=("Segoe UI", 8), anchor="w").pack(
            fill="x", padx=10, pady=6)

        # Buttons
        btn_row = tk.Frame(self, bg=_get_c("SURFACE"), padx=28)
        btn_row.pack(fill="x", pady=(0, 20))
        btn(btn_row, "  Dang ky  ", self._submit,
            icon="✅").pack(side="left")
        btn(btn_row, "Huy", self.destroy,
            variant="ghost").pack(side="left", padx=10)

    def _submit(self) -> None:
        import re
        v = {k: var.get().strip() for k, var in self._vars.items()}
        # Required fields
        for field, label in (("full_name", "Ho va ten"),
                             ("username",  "Ten dang nhap"),
                             ("email",     "Email"),
                             ("phone",     "So dien thoai"),
                             ("password",  "Mat khau")):
            if not v[field]:
                messagebox.showwarning("Thieu thong tin",
                                       f"Vui long nhap: {label}.",
                                       parent=self)
                return
        # Password minimum length
        if len(v["password"]) < 6:
            messagebox.showwarning("Mat khau qua ngan",
                                   "Mat khau phai co it nhat 6 ky tu.",
                                   parent=self)
            return
        if v["password"] != v["confirm"]:
            messagebox.showerror("Loi", "Mat khau xac nhan khong khop.",
                                 parent=self)
            return
        # Email format
        if not re.match(r"^[\w.+-]+@[\w-]+\.[\w.]+$", v["email"]):
            messagebox.showwarning("Email khong hop le",
                                   "Dinh dang email sai. Vi du: ten@truong.edu.vn",
                                   parent=self)
            return
        # Username: letters/digits/underscore only
        if not re.match(r"^[\w]+$", v["username"]):
            messagebox.showwarning("Ten dang nhap khong hop le",
                                   "Ten dang nhap chi duoc chua chu, so, dau _.",
                                   parent=self)
            return
        try:
            self.auth_ctrl.register(
                full_name=v["full_name"],
                username=v["username"],
                email=v["email"],
                phone=v["phone"],
                password=v["password"],
                role=v["role"],
            )
        except ValueError as exc:
            messagebox.showerror("Dang ky that bai", str(exc), parent=self)
            return
        messagebox.showinfo(
            "Dang ky thanh cong",
            f"Tai khoan '{v['username']}' da duoc tao!\n"
            "Ban co the dang nhap ngay bay gio.",
            parent=self)
        self.destroy()


# ── Forgot-password dialog ────────────────────────────────────────────────────

class ForgotPasswordDialog(tk.Toplevel):
    def __init__(self, master: tk.Misc, auth_controller: Any) -> None:
        super().__init__(master)
        self.auth_ctrl = auth_controller
        self.title("Quen mat khau")
        self.resizable(False, False)
        self.configure(bg=_get_c("SURFACE"))
        self.transient(master)  # type: ignore[arg-type]
        self.grab_set()
        self._step = 1          # 1 = verify identity, 2 = set new password
        self._verified_user: Any = None
        self._vars = {k: tk.StringVar() for k in
                      ("username", "email", "new_pw", "confirm_pw")}
        self._build_step1()
        self.after(100, self._center)

    def _center(self):
        self.update_idletasks()
        pw = self.master.winfo_rootx() + self.master.winfo_width()  // 2
        ph = self.master.winfo_rooty() + self.master.winfo_height() // 2
        w, h = self.winfo_width(), self.winfo_height()
        self.geometry(f"+{pw - w//2}+{ph - h//2}")

    def _clear(self):
        for w in self.winfo_children():
            w.destroy()

    def _build_step1(self) -> None:
        self._clear()
        # Header
        hdr = tk.Frame(self, bg=_get_c("ACCENT"), padx=24, pady=14)
        hdr.pack(fill="x")
        tk.Label(hdr, text="🔑  Quen mat khau",
                 bg=_get_c("ACCENT"), fg="white",
                 font=("Segoe UI", 13, "bold")).pack(anchor="w")
        tk.Label(hdr, text="Buoc 1: Xac minh danh tinh",
                 bg=_get_c("ACCENT"), fg="#bae6fd",
                 font=("Segoe UI", 9)).pack(anchor="w")

        body = tk.Frame(self, bg=_get_c("SURFACE"), padx=28, pady=20)
        body.pack(fill="x")

        info = tk.Frame(body, bg=_get_c("INFO_BG"),
                        highlightthickness=1, highlightbackground=_get_c("BORDER"))
        info.pack(fill="x", pady=(0, 14))
        tk.Label(info,
                 text="  Nhap ten dang nhap va email da dang ky\n"
                      "  de xac minh danh tinh cua ban.",
                 bg=_get_c("INFO_BG"), fg=_get_c("ACCENT"),
                 font=("Segoe UI", 9), justify="left", anchor="w").pack(
            fill="x", padx=8, pady=8)

        _labeled_entry(body, "TEN DANG NHAP",    self._vars["username"])
        _labeled_entry(body, "EMAIL DA DANG KY", self._vars["email"])

        btn_row = tk.Frame(self, bg=_get_c("SURFACE"), padx=28)
        btn_row.pack(fill="x", pady=(0, 20))
        btn(btn_row, "  Xac minh  ", self._verify_step1,
            variant="primary", icon="🔍").pack(side="left")
        btn(btn_row, "Huy", self.destroy,
            variant="ghost").pack(side="left", padx=10)

    def _verify_step1(self) -> None:
        username = self._vars["username"].get().strip()
        email    = self._vars["email"].get().strip()
        if not username or not email:
            messagebox.showwarning("Thieu thong tin",
                                   "Vui long nhap day du thong tin.",
                                   parent=self)
            return
        user = self.auth_ctrl.find_account_for_reset(username, email)
        if user is None:
            messagebox.showerror(
                "Xac minh that bai",
                "Khong tim thay tai khoan voi ten dang nhap va email nay.\n"
                "Kiem tra lai thong tin va thu lai.",
                parent=self)
            return
        self._verified_user = user
        self._build_step2()

    def _build_step2(self) -> None:
        self._clear()
        # Header
        hdr = tk.Frame(self, bg=_get_c("SUCCESS"), padx=24, pady=14)
        hdr.pack(fill="x")
        tk.Label(hdr, text="✅  Xac minh thanh cong",
                 bg=_get_c("SUCCESS"), fg="white",
                 font=("Segoe UI", 13, "bold")).pack(anchor="w")
        tk.Label(hdr, text="Buoc 2: Dat mat khau moi",
                 bg=_get_c("SUCCESS"), fg="#bbf7d0",
                 font=("Segoe UI", 9)).pack(anchor="w")

        body = tk.Frame(self, bg=_get_c("SURFACE"), padx=28, pady=20)
        body.pack(fill="x")

        success_box = tk.Frame(body, bg=_get_c("SUCCESS_BG"),
                               highlightthickness=1,
                               highlightbackground=_get_c("BORDER"))
        success_box.pack(fill="x", pady=(0, 14))
        name = getattr(self._verified_user, "full_name", "")
        tk.Label(success_box,
                 text=f"  Da xac minh: {name}",
                 bg=_get_c("SUCCESS_BG"), fg=_get_c("SUCCESS"),
                 font=("Segoe UI", 9, "bold"), anchor="w").pack(
            fill="x", padx=8, pady=8)

        _labeled_entry(body, "MAT KHAU MOI * (min 6)",
                       self._vars["new_pw"], show="*")
        _labeled_entry(body, "XAC NHAN MAT KHAU MOI",
                       self._vars["confirm_pw"], show="*")

        btn_row = tk.Frame(self, bg=_get_c("SURFACE"), padx=28)
        btn_row.pack(fill="x", pady=(0, 20))
        btn(btn_row, "  Luu mat khau moi  ", self._save_password,
            variant="success", icon="💾").pack(side="left")
        btn(btn_row, "Huy", self.destroy,
            variant="ghost").pack(side="left", padx=10)

    def _save_password(self) -> None:
        new_pw     = self._vars["new_pw"].get().strip()
        confirm_pw = self._vars["confirm_pw"].get().strip()
        if new_pw != confirm_pw:
            messagebox.showerror("Loi",
                                 "Mat khau xac nhan khong khop.", parent=self)
            return
        try:
            self.auth_ctrl.reset_password(
                username=self._verified_user.username,
                email=self._verified_user.email,
                new_password=new_pw,
            )
        except ValueError as exc:
            messagebox.showerror("Loi", str(exc), parent=self)
            return
        messagebox.showinfo(
            "Thanh cong",
            "Mat khau da duoc doi thanh cong!\n"
            "Vui long dang nhap lai bang mat khau moi.",
            parent=self)
        self.destroy()


# ── Login frame ───────────────────────────────────────────────────────────────

class LoginFrame(tk.Frame):
    def __init__(self, master: tk.Misc, on_login: Any,
                 auth_controller: Any = None) -> None:
        super().__init__(master, bg=_get_c("BG"))
        self.on_login   = on_login
        self.auth_ctrl  = auth_controller
        self.username_var = tk.StringVar()
        self.password_var = tk.StringVar()
        self._build()

    def _build(self) -> None:
        self.columnconfigure(0, weight=1)
        self.columnconfigure(1, weight=1)
        self.rowconfigure(0, weight=1)

        # ── Left animated branding panel ──────────────────────────────────────
        left = tk.Frame(self, bg=_get_lg_c("_LG_BG"))
        left.grid(row=0, column=0, sticky="nsew")

        cv = tk.Canvas(left, bg=_get_lg_c("_LG_BG"), highlightthickness=0)
        cv.pack(fill="both", expand=True)

        _particles: list[dict[str, Any]] = []
        _anim_ref: list[Any] = [None]

        def _init_particles(w: int, h: int) -> None:
            _particles.clear()
            # Indigo / Violet spectrum or Dark Slate
            _COLORS = [_get_lg_c("_LG_MID"), _get_lg_c("_LG_GLOW"), _get_lg_c("_LG_MUTED"), "#6366f1", "#818cf8"]
            for _ in range(15):
                _particles.append({
                    'x':   _rnd.uniform(0, w),
                    'y':   _rnd.uniform(0, h),
                    'r':   _rnd.uniform(w * 0.04, w * 0.18),
                    'dy':  _rnd.uniform(0.18, 0.60),
                    'dx':  _rnd.uniform(-0.18, 0.18),
                    'col': _rnd.choice(_COLORS),
                })

        def _animate() -> None:
            if not cv.winfo_exists():
                return
            w, h = cv.winfo_width(), cv.winfo_height()
            if w < 20:
                _anim_ref[0] = cv.after(120, _animate)
                return
            if not _particles:
                _init_particles(w, h)

            cv.delete("all")

            # Floating orbs (draw first so text renders on top)
            for p in _particles:
                p['y'] -= p['dy']
                p['x'] += p['dx']
                if p['y'] + p['r'] < 0:
                    p['y'] = h + p['r']
                    p['x'] = _rnd.uniform(0, w)
                if p['x'] < -p['r']:
                    p['x'] = w + p['r']
                elif p['x'] > w + p['r']:
                    p['x'] = -p['r']
                r = p['r']
                cv.create_oval(p['x'] - r, p['y'] - r,
                               p['x'] + r, p['y'] + r,
                               fill=p['col'], outline='')

            # ── Static branding content ───────────────────────────────────────
            cx = w // 2

            # University icon glow ring
            cv.create_oval(cx - 52, h * .27 - 52, cx + 52, h * .27 + 52,
                           fill=_get_lg_c("_LG_MID"), outline=_get_lg_c("_LG_MUTED"), width=2)
            cv.create_text(cx, h * .27, text="\U0001f3eb",
                           font=("Segoe UI", 44), fill="white",
                           anchor="center")

            cv.create_text(cx, h * .42,
                           text="HE THONG",
                           font=("Segoe UI", 22, "bold"), fill="white",
                           anchor="center")
            cv.create_text(cx, h * .49,
                           text="QUAN LY DAT PHONG HOC",
                           font=("Segoe UI", 11), fill=_get_lg_c("_LG_TEXT"),
                           anchor="center")

            # Divider
            cv.create_line(cx - 80, h * .56, cx + 80, h * .56,
                           fill=_get_lg_c("_LG_MUTED"), width=2)

            # Feature bullets
            for i, feat in enumerate([
                "✦  Dat phong tuc thi",
                "✦  Quan ly lich theo tuan",
                "✦  Bao cao & thong ke",
            ]):
                cv.create_text(cx, h * .62 + i * h * .065,
                               text=feat, font=("Segoe UI", 9),
                               fill=_get_lg_c("_LG_TEXT"), anchor="center")

            cv.create_text(cx, h * .88, text="v2.0  \u2022  Nhom 24",
                           font=("Segoe UI", 8), fill=_get_lg_c("_LG_MUTED"),
                           anchor="center")

            _anim_ref[0] = cv.after(38, _animate)

        def _on_configure(_e: Any = None) -> None:
            if _anim_ref[0] is None:
                _animate()

        def _on_destroy(_e: Any = None) -> None:
            if _anim_ref[0] is not None:
                try:
                    cv.after_cancel(_anim_ref[0])
                except Exception:
                    pass
                _anim_ref[0] = None

        cv.bind("<Configure>", _on_configure)  # type: ignore[arg-type]
        cv.bind("<Destroy>",   _on_destroy)    # type: ignore[arg-type]

        # ── Right form panel ──────────────────────────────────────────────────
        right = tk.Frame(self, bg=_get_c("SURFACE"))
        right.grid(row=0, column=1, sticky="nsew")

        # Indigo accent stripe on right side
        tk.Frame(right, bg=_get_c("ACCENT"), height=4).pack(fill="x")

        # Subtle decorative background canvas
        _decor_cv = tk.Canvas(right, bg=_get_c("SURFACE"), highlightthickness=0)
        _decor_cv.place(relx=0, rely=0, relwidth=1, relheight=1)

        def _draw_decor(_e: Any = None) -> None:
            if not _decor_cv.winfo_exists():
                return
            _decor_cv.delete("all")
            W = _decor_cv.winfo_width() or 480
            H = _decor_cv.winfo_height() or 640
            # Large soft circle top-right
            _decor_cv.create_oval(W - 120, -60, W + 60, 120,
                                   fill=_get_c("INFO_BG"), outline="")
            # Medium circle bottom-left
            _decor_cv.create_oval(-80, H - 120, 80, H + 40,
                                   fill=_get_c("INFO_BG"), outline="")
            # Small accent dot
            _decor_cv.create_oval(W - 55, H - 55, W - 35, H - 35,
                                   fill=_get_c("ACCENT"), outline="")
            # Horizontal subtle stripe near bottom
            _decor_cv.create_rectangle(0, H - 8, W, H,
                                        fill=_get_c("INFO_BG"), outline="")

        _decor_cv.bind("<Configure>", _draw_decor)
        _decor_cv.after(30, _draw_decor)

        card = tk.Frame(right, bg=_get_c("SURFACE"))
        card.place(relx=0.5, rely=0.5, anchor="center")
        self._login_card = card

        # Heading
        tk.Label(card, text="Chao mung tro lai!", bg=_get_c("SURFACE"), fg=_get_c("TEXT"),
                 font=("Segoe UI", 26, "bold")).pack(anchor="w")
        tk.Label(card, text="Dang nhap de quan ly dat phong hoc",
                 bg=_get_c("SURFACE"), fg=_get_c("MUTED"),
                 font=("Segoe UI", 10)).pack(anchor="w", pady=(4, 24))

        # ── Username field ────────────────────────────────────────────────────
        tk.Label(card, text="TEN DANG NHAP", bg=_get_c("SURFACE"), fg=_get_c("MUTED"),
                 font=("Segoe UI", 8, "bold")).pack(anchor="w")
        un_wrap = tk.Frame(card, bg=_get_c("BORDER"), padx=1, pady=1)
        un_wrap.pack(fill="x", pady=(3, 16))

        # Icon + entry
        un_inner = tk.Frame(un_wrap, bg=_get_c("SURFACE"))
        un_inner.pack(fill="x")
        tk.Label(un_inner, text="👤", bg=_get_c("SURFACE"), fg=_get_c("MUTED"),
                 font=("Segoe UI", 13)).pack(side="left", padx=(10, 4), pady=8)
        un_e = tk.Entry(un_inner, textvariable=self.username_var, width=28,
                        font=F_INPUT, relief="flat", bg=_get_c("SURFACE"), fg=_get_c("TEXT"),
                        insertbackground=_get_c("ACCENT"))
        un_e.pack(side="left", padx=(0, 10), pady=9, fill="x", expand=True)
        un_e.focus_set()
        self._bind_focus(un_e, un_wrap)

        # ── Password field + eye toggle ────────────────────────────────────────
        pw_hdr = tk.Frame(card, bg=_get_c("SURFACE"))
        pw_hdr.pack(fill="x")
        tk.Label(pw_hdr, text="MAT KHAU", bg=_get_c("SURFACE"), fg=_get_c("MUTED"),
                 font=("Segoe UI", 8, "bold")).pack(side="left")
        forgot_lbl = tk.Label(pw_hdr, text="Quen mat khau?",
                              bg=_get_c("SURFACE"), fg=_get_c("ACCENT"),
                              font=("Segoe UI", 9, "underline"),
                              cursor="hand2")
        forgot_lbl.pack(side="right")
        forgot_lbl.bind("<Button-1>", lambda _=None: self._open_forgot())

        pw_wrap = tk.Frame(card, bg=_get_c("BORDER"), padx=1, pady=1)
        pw_wrap.pack(fill="x", pady=(3, 24))

        pw_inner = tk.Frame(pw_wrap, bg=_get_c("SURFACE"))
        pw_inner.pack(fill="x")
        tk.Label(pw_inner, text="🔒", bg=_get_c("SURFACE"), fg=_get_c("MUTED"),
                 font=("Segoe UI", 13)).pack(side="left", padx=(10, 4), pady=8)
        pw_e = tk.Entry(pw_inner, textvariable=self.password_var, show="*",
                        width=24, font=F_INPUT, relief="flat",
                        bg=_get_c("SURFACE"), fg=_get_c("TEXT"),
                        insertbackground=_get_c("ACCENT"))
        pw_e.pack(side="left", padx=(0, 4), pady=9, fill="x", expand=True)

        # Eye toggle
        _show_pw = [False]
        def _toggle_eye():
            _show_pw[0] = not _show_pw[0]
            pw_e.config(show="" if _show_pw[0] else "*")
            eye_btn.config(text="🙈" if _show_pw[0] else "👁")
        eye_btn = tk.Button(pw_inner, text="👁", bg=_get_c("SURFACE"), fg=_get_c("MUTED"),
                            font=("Segoe UI", 11), relief="flat", bd=0,
                            cursor="hand2", command=_toggle_eye,
                            activebackground=_get_c("SURFACE"))
        eye_btn.pack(side="right", padx=(0, 8))

        self._bind_focus(pw_e, pw_wrap)

        # ── Inline error label ───────────────────────────────────────────────
        self._err_lbl = tk.Label(card, text="", bg=_get_c("SURFACE"), fg=_get_c("DANGER"),
                                  font=("Segoe UI", 9, "bold"),
                                  wraplength=320, justify="left")
        self._err_lbl.pack(fill="x", pady=(0, 6))

        # ── Login button ──────────────────────────────────────────────────────
        self._login_btn = btn(card, "  DANG NHAP  →", self._submit)
        self._login_btn.config(width=30, pady=12, font=("Segoe UI", 12, "bold"),
                 bg=_get_c("ACCENT"), activebackground=_get_c("SB_HOVER"))
        self._login_btn.pack(fill="x")

        # ── Register link ─────────────────────────────────────────────────────
        reg_row = tk.Frame(card, bg=_get_c("SURFACE"))
        reg_row.pack(pady=(18, 0))
        tk.Label(reg_row, text="Chua co tai khoan?  ",
                 bg=_get_c("SURFACE"), fg=_get_c("MUTED"),
                 font=("Segoe UI", 10)).pack(side="left")
        reg_lbl = tk.Label(reg_row, text="Dang ky ngay →",
                           bg=_get_c("SURFACE"), fg=_get_c("ACCENT"),
                           font=("Segoe UI", 10, "bold", "underline"),
                           cursor="hand2")
        reg_lbl.pack(side="left")
        reg_lbl.bind("<Button-1>", lambda _=None: self._open_register())

        un_e.bind("<Return>", lambda _=None: self._submit())
        pw_e.bind("<Return>", lambda _=None: self._submit())

    @staticmethod
    def _bind_focus(entry: tk.Entry, frame: tk.Frame) -> None:
        def on_in(_: Any) -> None:
            frame.config(bg=_get_c("ACCENT"))   # Indigo focus ring
        def on_out(_: Any) -> None:
            frame.config(bg=_get_c("BORDER"))
        entry.bind("<FocusIn>",  on_in)   # type: ignore[arg-type]
        entry.bind("<FocusOut>", on_out)  # type: ignore[arg-type]

    def _open_register(self) -> None:
        if self.auth_ctrl is None:
            messagebox.showinfo("Thong bao",
                                "Chuc nang dang ky chua duoc ket noi.")
            return
        RegisterDialog(self, self.auth_ctrl)

    def _open_forgot(self) -> None:
        if self.auth_ctrl is None:
            messagebox.showinfo("Thong bao",
                                "Chuc nang lay lai mat khau chua duoc ket noi.")
            return
        ForgotPasswordDialog(self, self.auth_ctrl)

    def _shake(self) -> None:
        """Briefly shake the login card horizontally to signal an error."""
        if not (hasattr(self, "_login_card") and self._login_card.winfo_exists()):
            return
        card = self._login_card
        offsets = [10, -10, 8, -8, 5, -5, 2, -2, 0]
        delay = 0
        for ox in offsets:
            self.after(delay, lambda o=ox, c=card: (
                c.winfo_exists() and c.place(relx=0.5, rely=0.5, anchor="center", x=o)
            ))
            delay += 45

    def _set_error(self, msg: str) -> None:
        if hasattr(self, "_err_lbl") and self._err_lbl.winfo_exists():
            self._err_lbl.config(text=msg)
        if msg:
            self._shake()

    def _submit(self) -> None:
        username = self.username_var.get().strip()
        password = self.password_var.get().strip()
        if not username or not password:
            self._set_error("⚠  Vui long nhap ten dang nhap va mat khau.")
            return
        self._set_error("")
        # Show loading state
        if hasattr(self, "_login_btn") and self._login_btn.winfo_exists():
            self._login_btn.config(text="  Dang kiem tra...  ", state="disabled",
                                    bg=_get_c("ACCENT"))
            self.update_idletasks()
        try:
            self.on_login(username, password)
        except PermissionError as e:
            # Lockout message from rate limiter
            try:
                if self._login_btn.winfo_exists():
                    self._login_btn.config(text="  DANG NHAP  →", state="normal",
                                            bg=_get_c("ACCENT"))
                    self._set_error(f"🔒  {e}")
            except Exception:
                pass
            self._shake()
        finally:
            # Restore button if frame still exists (= login failed / frame not destroyed)
            try:
                if self._login_btn.winfo_exists() and str(self._login_btn["state"]) == "disabled":
                    self._login_btn.config(text="  DANG NHAP  →", state="normal",
                                            bg=_get_c("ACCENT"))
                    self._set_error("✖  Sai ten dang nhap hoac mat khau, hoac tai khoan bi khoa.")
            except Exception:
                pass
