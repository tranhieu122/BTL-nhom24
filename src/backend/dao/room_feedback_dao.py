...
class RoomRatingDAO:
    ...
    def list_all(self, limit: int = 100) -> list[RoomRating]:
        """Fetch all recent ratings across all rooms."""
        conn = get_connection()
        rows = conn.execute(
            "SELECT * FROM room_ratings ORDER BY created_at DESC LIMIT ?", (limit,)
        ).fetchall()
        return [_row_to_rating(r) for r in rows]
...
class EquipmentReportDAO:
    ...
    def find_by_id(self, report_id: int) -> EquipmentReport | None:
        conn = get_connection()
        row = conn.execute(
            "SELECT * FROM equipment_reports WHERE id=?", (report_id,)
        ).fetchone()
        return _row_to_equip_report(row) if row else None
