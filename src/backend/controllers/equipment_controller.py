...
class EquipmentController:
    ...
    def resolve_report(self, report_id: int, fix_equipment: bool = False) -> None:
        """Mark a report as resolved. Optionally reset equipment status to 'Hoat dong'."""
        self.report_dao.update_status(report_id, "Da xu ly")
        if fix_equipment:
            report = self.report_dao.find_by_id(report_id)
            if report:
                self.update_status(report.equipment_id, "Hoat dong")
        _log.info("Equipment report %d resolved (fix_equipment=%s)",
                  report_id, fix_equipment)
