...
class BookingController:
    ...
    def available_slots(self, room_id: str, booking_date: str) -> list[str]:
        """Return list of free slots for a room on a given date, checking both bookings and recurring rules."""
        used = {b.slot for b in self.booking_dao.search(
                    room_id=room_id, date_from=booking_date, date_to=booking_date)
                if b.status != "Tu choi"}

        # Also check recurring schedule occurrences (Lich day)
        try:
            from dao.schedule_rule_dao import ScheduleRuleDAO
            occ_dao = ScheduleRuleDAO()
            # Query occurrences for the specific room and date
            occurrences = occ_dao.list_occurrences_by_date_range(booking_date, booking_date)
            ...
            for occ in occurrences:
                if occ.room_id == room_id and occ.status != "Huy":
                    slot = _SLOT_TIME_MAP.get((occ.start_time, occ.end_time))
                    if slot:
                        used.add(slot)
                    else:
                        # Handle non-standard slot times if any
                        # For now, we assume standard slots are the priority
                        pass
        except Exception:
            # Fallback to only one-off bookings if recurring logic fails
            pass
    ...
    def build_schedule(self, week_offset: int = 0, only_user_id: str = "") -> list[Schedule]:
        ...
        If only_user_id is provided, only returns one-off bookings for that user.
        """
        rows: list[Schedule] = []
        today = dt.date.today()
        ...
        # ── One-off bookings ─────────────────────────────────────────────────
        for b in self.booking_dao.list_all():
            if only_user_id and b.user_id != only_user_id:
                continue
            try:
