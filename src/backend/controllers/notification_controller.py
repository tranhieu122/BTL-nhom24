...
class NotificationController:
    ...
    def send_to_role(self, sender: User, role: str, title: str,
        ...
        targets = [u for u in user_list
                   if u.role == role and u.user_id != sender.user_id]
    ...
    def send_to_all(self, sender: User, title: str, message: str,
        ...
        targets = [u for u in user_list if u.user_id != sender.user_id]
        return self._bulk_send(sender, title, message, targets)
    ...
    def get_notifications(self, user_id: str, # type: ignore
                          unread_only: bool = False) -> list[dict]: # type: ignore
        notifs = self.notif_dao.list_for_user(user_id) # type: ignore
        if unread_only:
            notifs = [n for n in notifs if not n.get("is_read")] # type: ignore
        return notifs # type: ignore
    ...
    def delete_all_read(self, user_id: str) -> int:
        ...
        notifs = self.notif_dao.list_for_user(user_id) # pyright: ignore[reportUnknownMemberType, reportUnknownVariableType]
        read_ids = [n["id"] for n in notifs if n.get("is_read")] # type: ignore
        for nid in read_ids: # type: ignore
            self.notif_dao.delete(nid) # type: ignore
        _log.info("Deleted %d read notifications for user %s",
                  len(read_ids), user_id) # type: ignore
        return len(read_ids) # type: ignore
